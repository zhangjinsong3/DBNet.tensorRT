...
def crop_bbox(img_path, result):
    ...
    for point in result:
        ...
        crops.append(perspective_transform(point, img_path))
    return crops
...
def perspective_transform(box, original_img):
    ...
    # /   /
    # 3---2
    :param box: list of points
    :param original_img: original image
    :return:
    """
    import math
    # 获取画框宽高(x=dst_W,y=dst_H)
    if isinstance(box, list):
        dst_W = math.ceil(np.sqrt((box[3][1] - box[2][1])**2 + (box[3][0] - box[2][0])**2))
        dst_H = math.ceil(np.sqrt((box[3][1] - box[0][1])**2 + (box[3][0] - box[0][0])**2))
    ...
    pts2 = np.float32([[0, 0], [int(dst_W+1), 0], [int(dst_W+1), int(dst_H+1)], [0, int(dst_H+1)]])

    # 生成透视变换矩阵；进行透视变换
    M = cv2.getPerspectiveTransform(pts1, pts2)
    result_img = cv2.warpPerspective(original_img, M, (int(dst_W+3),int(dst_H+1)))
    return result_img
