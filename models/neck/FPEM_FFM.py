...
class FPEM_FFM(nn.Module):
    ...
    def forward(self, x):
        ...
        # FFM
        # Notice: https://blog.csdn.net/foso1994/article/details/108881940
        size = [int(c2_ffm.size()[-2]), int(c2_ffm.size()[-1])]
        c5 = F.interpolate(c5_ffm, size)
        c4 = F.interpolate(c4_ffm, size)
        c3 = F.interpolate(c3_ffm, size)
...
class FPEM(nn.Module):
    ...
    def _upsample_add(self, x, y):
        # Notice: https://blog.csdn.net/foso1994/article/details/108881940
        size = [int(y.size()[2]), int(y.size()[3])]
        return F.interpolate(x, size=size) + y
