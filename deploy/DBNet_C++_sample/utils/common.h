...
#ifdef DEMO_TEST
#define TM_START() auto p_start = std::chrono::high_resolution_clock::now();
#define TM_COST(funname) auto p_end = std::chrono::high_resolution_clock::now();\
					float p_total = std::chrono::duration<float, std::milli>(p_end - p_start).count();\
					std::cout << funname << " cost time: " << p_total << " ms." << std::endl;
#else
#define TM_START()
#define TM_COST(funname) 
#endif		
